...
export default function Verify() {
    ...
    const handleSubmit = async (event) => {
        ...
        if (verificationCode === code) {
            try {
                ...
                const response = await axios.put('https://chirkut-server.vercel.app/api/v2/auth/verify', { email: userEmail });
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                const response = await axios.get(`https://chirkut-server.vercel.app/api/v2/auth/verify/${userEmail}`);
